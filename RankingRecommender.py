...
class RankingRecommender(Recommender):
    def __init__(self, sess, data, configs, logger):
        ...
        print('len(test_users): ', len(self.test_users), self.test_batches)
    ...
    def train_model_sohrml2(self):
        total_loss = 0.0
        train_ = ranking_sampler_sohrml2(self.data, self.neg_ratio)
        batch_len = math.ceil(train_[0]/self.train_batches)
        # Train
        for idx in range(self.train_batches):
            u_idx, i_idx, j_idx, v_idx, w_idx = train_[1][idx*batch_len:(idx+1)*batch_len], train_[2][idx*batch_len:(idx+1)*batch_len], train_[3][idx*batch_len:(idx+1)*batch_len], \
                train_[4][idx*batch_len:(idx+1)*batch_len], train_[5][idx*batch_len:(idx+1)*batch_len]
            train_dict = {self.u_idx: u_idx, self.i_idx: i_idx, self.j_idx: j_idx, self.v_idx: v_idx, self.w_idx: w_idx, self.is_train: 1}
            # Run
            _, loss_val = self.sess.run([self.train, self.loss], train_dict)
            total_loss += loss_val
        return total_loss/self.train_batches

...
class SoHRML(RankingRecommender):
    def __init__(self, sess, data, configs, logger):
        ...
        self.att_dropout, self.node_dropout, self.message_dropout = float(configs['att_dropout']), float(configs['node_dropout']), float(configs['message_dropout'])
        ...
        logger.info(' model_params: embed_size=%d, atten_size=%d, att_type=%d, mlp_type=%d, gat_layer_nums=%d, gamma=%s, reg1=%s, reg2=%s, margin=%s, node_dropout=%s, message_dropout=%s' % \
            (self.embed_size, self.atten_size, self.att_type, self.mlp_type, self.gat_layer_nums, self.gamma, self.reg1, self.reg2, self.margin, self.node_dropout, self.message_dropout) + \
                ', ' + self.model_params)
        self.item_set = tf.constant(list(self.data.item_set)) # For testing
        # Specify training model
        self.train_model = self.train_model_sohrml2
    ...
    def _create_params(self):
        ...
        with tf.name_scope('sohrml_params'):
            ...
            # GAT params
            self.gat_params = {}
            for idx in range(self.gat_layer_nums):
                self.gat_params['W_gat_'+str(idx)] = __create_w([self.embed_size, self.embed_size], 'W_gat_'+str(idx))
                self.gat_params['b_gat_'+str(idx)] = __create_b([self.embed_size], 'b_gat_'+str(idx))

            # MLP params
            self.mlp_params = {}
            for idx in range(self.mlp_type):
                self.mlp_params['W_mlp_'+str(idx)] = __create_w([2*self.embed_size, min(self.mlp_type-idx, 2)*self.embed_size], 'W_mlp_'+str(idx))
                self.mlp_params['b_mlp_'+str(idx)] = __create_b([min(self.mlp_type-idx, 2)*self.embed_size], 'b_mlp_'+str(idx))
    ...
    def _create_embeddings(self):
        with tf.name_scope('embeddings'):
            ...
            self.s_existed = tf.cast(tf.not_equal(self.v_idx, self.data.user_nums), tf.float32) # Invalid friends
            self.u_embed_s = tf.einsum('a,ab->ab', self.s_existed, tf.gather(self.P, self.u_idx))
            self.v_embed = tf.einsum('a,ab->ab', self.s_existed, tf.gather(self.P, self.v_idx))
            self.w_embed = tf.einsum('a,ab->ab', self.s_existed, tf.gather(self.P, self.w_idx))
    ...
    def _get_att_scores(self):
        with tf.name_scope('att_scores'):
            ...
            if self.att_type == 0:
                ...
                self.att_scores_i_batch = tf.nn.relu(tf.nn.dropout(tf.einsum('ab,ab->a', r_embed_i, c_embed_i), keep_prob=1-self.att_dropout))
                self.att_scores_s_batch = tf.nn.relu(tf.nn.dropout(tf.einsum('ab,ab->a', r_embed_s, c_embed_s), keep_prob=1-self.att_dropout))
            elif self.att_type == 2:
                self.att_scores_i_batch = tf.einsum('ac,c->a', tf.nn.relu( \
                    tf.nn.dropout(tf.einsum('ab,bc->ac', tf.concat([r_embed_i, c_embed_i], 1), self.W) + self.b, keep_prob=1-self.att_dropout)), \
                        self.h)
                self.att_scores_s_batch = tf.einsum('ac,c->a', tf.nn.relu( \
                    tf.nn.dropout(tf.einsum('ab,bc->ac', tf.concat([r_embed_s, c_embed_s], 1), self.W) + self.b, keep_prob=1-self.att_dropout)), \
                        self.h)
    ...
    def _build_mlp(self, nbr_embed_1, nbr_embed_2, t_existed=None, is_test=False):
        with tf.name_scope('build_mlp'):
            if self.mlp_type == 0: # Element-wise product
                if is_test:
                    r_vec = tf.einsum('ac,bc->abc', nbr_embed_1, nbr_embed_2)
                else:
                ...
                for idx in range(self.mlp_type):
                    r_vec = tf.nn.relu(tf.matmul(r_vec, self.mlp_params['W_mlp_'+str(idx)]) + self.mlp_params['b_mlp_'+str(idx)])
            if t_existed is not None:
                r_vec = tf.einsum('a,ab->ab', t_existed, r_vec)
            return r_vec
    ...
    def _build_gat(self, is_test=False):
        with tf.name_scope('build_gat'):
            ...
            self.j_nbr_embed = tf.gather(i_g_embed_i, self.j_idx)
            if is_test:
                self.i_nbr_embed = tf.gather(i_g_embed_i, self.item_set)
            else:
                self.i_nbr_embed = tf.gather(i_g_embed_i, self.i_idx)
            # Social domain
            # self.u_nbr_embed_s = tf.gather(u_g_embed_s, self.u_idx_s)
            self.u_nbr_embed_s = tf.gather(u_g_embed_s, self.u_idx)
    ...
    def _create_inference(self):
        with tf.name_scope('inference'):
            ...
            self.uv_vec = self._build_mlp(self.u_nbr_embed_s, self.v_nbr_embed, t_existed=self.s_existed)
            self.uw_vec = self._build_mlp(self.u_nbr_embed_s, self.w_nbr_embed, t_existed=self.s_existed)
            ...
            def _clip_grad(grad_):
                if grad_ is not None:
                    grad_ = tf.clip_by_norm(grad_, 1.0)
                return grad_
            clipped_gradients = [(_clip_grad(grad), var) for grad, var in gradients]
            # Update variables
            self.train = self.optimizer.apply_gradients(clipped_gradients)
    ...
    def _predict(self):
        if self.configs['data.split_way'] == 'loo' or self.neg_samples > 0:
            ...
            self._build_gat(is_test=True)
            ui_vec_t = self._build_mlp(self.u_nbr_embed_i, self.i_nbr_embed, is_test=True)
            self.pre_scores = tf.reduce_sum(tf.square(tf.expand_dims(self.u_embed_i, 1) + ui_vec_t - tf.expand_dims(i_embed_t, 0)), 2)

    def _save_model(self):
        var_list = {'sohrml_params/P': self.P, 'sohrml_params/Q': self.Q}
